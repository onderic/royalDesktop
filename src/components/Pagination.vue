<template>
    <div class="py-10 text-center dark:bg-dark">
      <ul class="flex items-center justify-center gap-2">
        <li>
          <button
            @click="prevPage"
            :class="{'cursor-not-allowed': currentPage === 1, 'border-stroke': true, 'bg-white': true}"
            class="flex h-10 min-w-10 items-center justify-center rounded-lg border px-2 text-base font-medium text-dark hover:bg-gray-1 dark:border-white/10 dark:bg-white/5 dark:text-white dark:hover:bg-white/10"
          >
            <span>
              <svg
                width="20"
        ...
        <li v-for="page in pages" :key="page">
          <button
            @click="setPage(page)"
            :class="{'border-[#1d4ed8] bg-[#1d4ed8] text-white': page === currentPage, 'border-stroke bg-white text-dark hover:bg-gray-1': page !== currentPage}"
            class="flex h-10 min-w-10 items-center justify-center rounded-lg border px-2 text-base font-medium hover:bg-gray-1 dark:border-white/10 dark:bg-white/5 dark:text-white dark:hover:bg-white/10"
          >
            {{ page }}
          </button>
        </li>
        <li>
          <button
            @click="nextPage"
            :class="{'cursor-not-allowed': currentPage === totalPages, 'border-stroke': true, 'bg-white': true}"
            class="flex h-10 min-w-10 items-center justify-center rounded-lg border px-2 text-base font-medium text-dark hover:bg-gray-1 dark:border-white/10 dark:bg-white/5 dark:text-white dark:hover:bg-white/10"
          >
            <span>
              <svg
  ...
  const props = defineProps(['currentPage', 'totalPages', 'pages']);
  
  const emit = defineEmits(['setPage', 'prevPage', 'nextPage']);
